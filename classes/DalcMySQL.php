<?
    ...
    class DalcMySQL implements \DbaInterface{
		...
		public function connect(){
		    ...
		    $link = mysql_connect('localhost', $this->php_spec->db_user, $this->php_spec->db_password);
		    if (!$link) {
			    die('Not connected : ' . mysql_error());
		    }

		    mysql_set_charset('utf8', $link); // turkce harfler icin

		    // make foo the current db
		    //$db_selected = mysql_select_db('csy_develop', $link);
		    $db_selected = mysql_select_db('minyay', $link);

		    if (!$db_selected) {
			    die ('Can\'t use minyay : ' . mysql_error());
		    }
